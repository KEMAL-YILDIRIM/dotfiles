local FileHistory = function()
	...
	while i <= #result do
		...
		if hash and date then
			...
			local commit_path = (i <= #result and not result[i]:match("^%x+%s+%d%d%d%d%-%d%d%-%d%d")) and result[i]
				or file_path
		...
		opts = {
			...
			on_attach = function(bufnr)
				...
				vim.keymap.set(
					...
					{ desc = "Git Signs: blame current line" }
				)
				...
				vim.keymap.set(
					...
					{ desc = "Gitsigns select hunk", silent = true }
				)
